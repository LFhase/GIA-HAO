...
class SPEIT(object):
    ...
    def __init__(self,
                 ...
                 inject_mode='multi-layer', # multi-layer is the original proposal
                 device='cpu',
                 early_stop=False,
                 verbose=True):
        self.device = device
    ...
    def injection(self, adj, n_inject, n_node, target_idx, target_node=None, mode='random-inter'):
        ...
        if mode == 'random':
            ...
            target_idx = target_idx.cpu()
            n_test = target_idx.shape[0]
            adj_inject = inject(torch.arange(n_test), n_inject, n_test, mode)
    ...
    def update_features(self, model, adj_attack, features, features_attack, origin_labels, target_idx):
        ...
        for i in range(n_epoch):
            ...
            test_score = metric.eval_acc(pred[:n_total][target_idx],
